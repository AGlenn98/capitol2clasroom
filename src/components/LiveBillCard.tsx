...
export function LiveBillCard({
  bill,
  className
}: LiveBillCardProps) {
  return <Link to={`/advocacy/bill/${bill.bill_id}`} className={cn("group block bg-card border border-border/40 rounded-2xl p-5 transition-all hover:border-primary/30 hover:shadow-md", className)}>
      ...
      <h3 className="text-sm tracking-wide mb-2 group-hover:text-primary transition-colors line-clamp-2 font-sans">
