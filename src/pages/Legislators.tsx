...
export default function Legislators() {
  ...
  const filteredLegislators = (legislators || []).filter(leg => {
    const matchesSearch = searchQuery === "" || leg.name.toLowerCase().includes(searchQuery.toLowerCase()) || leg.district.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    const matchesChamber = chamberFilter === "All" || chamberFilter === "House" && isRep || chamberFilter === "Senate" && isSenator;
        ...
        <div className="container">
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <Input type="search" placeholder="Search by name or district..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9" />
            </div>
            <div className="flex items-center gap-3 overflow-x-auto pb-2 md:pb-0">
              <Filter className="w-4 h-4 text-muted-foreground shrink-0" />
              <div className="flex gap-2">
                {(["All", "R", "D"] as const).map(party => <Button key={party} variant={partyFilter === party ? "default" : "outline"} size="sm" onClick={() => setPartyFilter(party)} className={cn("shrink-0", party === "R" && partyFilter === party && "bg-republican hover:bg-republican/90", party === "D" && partyFilter === party && "bg-democrat hover:bg-democrat/90")}>
                    {party === "All" ? "All Parties" : party === "R" ? "Republican" : "Democrat"}
                  </Button>)}
              </div>
              <div className="h-6 w-px bg-border" />
              <div className="flex gap-2">
                {(["All", "House", "Senate"] as const).map(chamber => <Button key={chamber} variant={chamberFilter === chamber ? "secondary" : "ghost"} size="sm" onClick={() => setChamberFilter(chamber)} className="shrink-0">
      ...
      <section className="py-8">
        <div className="container">
          {isLoading ? <div className="flex flex-col items-center justify-center py-12">
            ...
            </div> : error ? <div className="text-center py-12 bg-destructive/5 border border-destructive/20 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredLegislators.map(legislator => <Link key={legislator.people_id} to={`/legislators/${legislator.people_id}`}>
                    <Card className="h-full hover:border-primary/50 hover:shadow-md transition-all">
                      <CardContent className="p-4">
                        <div className="flex items-start gap-4">
                          {legislator.photo_url ? <div className={cn("w-12 h-12 rounded-full overflow-hidden shrink-0 border-3", legislator.party === "R" ? "border-republican" : "border-democrat")}>
                              <img src={legislator.photo_url} alt={legislator.name} className="w-full h-full object-cover" onError={e => {
                        ...
                        if (parent) {
                          parent.innerHTML = `<span class="w-full h-full flex items-center justify-center text-lg font-bold ${legislator.party === 'R' ? 'bg-republican-muted text-republican' : 'bg-democrat-muted text-democrat'}">${legislator.first_name[0]}${legislator.last_name[0]}</span>`;
                            ...
                            </div> : <div className={cn("w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold shrink-0 border-3", legislator.party === "R" ? "bg-republican-muted text-republican border-republican" : "bg-democrat-muted text-democrat border-democrat")}>
                              {legislator.first_name[0]}{legislator.last_name[0]}
                            </div>}
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold truncate font-sans">
                              {legislator.role}. {legislator.name}
                            </h3>
                            <div className="flex items-center gap-2 mt-1">
                              <Badge variant="outline" className={cn("text-xs", legislator.party === "R" ? "border-republican/30 text-republican" : "border-democrat/30 text-democrat")}>
              ...
              {filteredLegislators.length === 0 && <div className="text-center py-12 bg-card border border-border rounded-lg">
